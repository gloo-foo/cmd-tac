...
func TestTac_ManyLines(t *testing.T) {
	input := "a\nb\nc\nd\ne\nf\ng\nh\ni\nj\n"
	lines, err := testable.TestLines(command.Tac(), run.Input(input))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTac_IgnoresPositionalArguments(t *testing.T) {
	// Tac reads from the pipeline: positional arguments are classified but not
	// consumed, and must not change the line-reversal behavior.
	lines, err := testable.TestLines(command.Tac(gloo.File("unused.txt")), "a\nb\n")
	if err != nil {
		t.Fatal(err)
	}
	if len(lines) != 2 || lines[0] != "b" || lines[1] != "a" {
		t.Fatalf("got %v, want [b a]", lines)
	}
}
...
func TestTac_TableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lines, err := testable.TestLines(command.Tac(), run.Input(tt.input))
			if err != nil {
				t.Fatal(err)
			}
