...
Compatible with GNU `tac` for the common case — reversing the line order of stdin or a single file is byte-identical. Multiple file operands and the `-s` record separator diverge by design (verified in the `yupsh/bin-tac` Docker integration harness against Debian coreutils).
